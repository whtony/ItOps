...
class LocalTimes(object):
    ...
    def timestamp13_to_strtime(timestamp):
    ...
    def timestamp10_to_strtime(timestamp):
        """将 10 位整数的毫秒时间戳转化成本地普通时间 (字符串格式)
        :param timestamp: 10 位整数的毫秒时间戳 (1552563153)
        :return: 返回字符串格式 {str}'2016-02-25 20:21:04.242000'
        """
        # local_str_time = datetime.fromtimestamp(timestamp).strftime('%Y-%m-%d %H:%M:%S.%f')
        local_str_time = datetime.strftime('%Y-%m-%d %H:%M:%S',timestamp)
        return local_str_time
    def timestamp13_to_datetime(timestamp):
    ...
    def datetime_to_strtime(datetime_obj):
        ...
        return local_str_time
    def datetime_to_timestamp13(datetime_obj):
        """将本地(local) datetime 格式的时间 (含毫秒) 转为毫秒时间戳
        :param datetime_obj: {datetime}2016-02-25 20:21:04.242000
        :return: 13 位的毫秒时间戳  1456402864242
        """
        local_timestamp = float(time.mktime(datetime_obj.timetuple()) * 1000.0 + datetime_obj.microsecond / 1000.0)
    ...
    def strtime_to_timestamp13(local_timestr):
        """将本地时间 (字符串格式，含毫秒) 转为 13 位整数的毫秒时间戳
        :param local_timestr: {str}'2016-02-25 20:21:04.242'
        :return: 1456402864242
        """
        local_datetime = LocalTimes.strtime_to_datetime(local_timestr)
        timestamp = LocalTimes.datetime_to_timestamp(local_datetime)
        return timestamp
    def current_datetime():
        ...
        # 当前时间：字符串格式
        local_strtime_now = LocalTimes.datetime_to_strtime(local_datetime_now)
        # 当前时间：时间戳格式 13位整数
        local_timestamp_now = LocalTimes.datetime_to_timestamp(local_datetime_now)
        return local_datetime_now, local_strtime_now, local_timestamp_now
    if __name__ == '__main__':
        pass

# time_str = '2016-02-25 20:21:04.242'
# timestamp1 = LocalTimes.strtime_to_timestamp(time_str)
# datetime1 = LocalTimes.strtime_to_datetime(time_str)
# time_str2 = LocalTimes.datetime_to_strtime(datetime1)
# timestamp2 = LocalTimes.datetime_to_timestamp(datetime1)
# datetime3 = LocalTimes.timestamp_to_datetime(timestamp2)
# time_str3 = LocalTimes.timestamp_to_strtime(timestamp2)
# current_time = LocalTimes.current_datetime()
